...
function UpdateGenesNetwork() {
    ...
    bubblechart.on("click", function (d, i) {
        ...
        if(store_protein.length>1){
            intersect.forEach(function (d) { d3.select('#g'+d).style("fill","black").attr("d",d3.svg.symbol().size(1000)) })
        }else{
            intersect.forEach(function (d) { d3.select('#g'+d).attr("d",d3.svg.symbol().size(340)) })
        }
